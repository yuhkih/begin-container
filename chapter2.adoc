...
`レポジトリ名` だけでは、イメージを任意に区別できないので、 `レポジトリ` 名 + `タグ` 名の形で `docker` コマンドの引数に指定します。

例外として `docker start nginx`  のように `タグ` を省略した場合、自動的に `docker start nginx:latest` のように `latest` が付加されて解釈する事があります。

但し `latest` の `タグ` は、 `イメージ` を作成した時に、`タグ` を省略した時にデフォルトで使われる値で、使わない事もできます。そのため、`タグ` を省略した時に `latest` の `タグ` がついた `イメージ` が存在しない時はエラーになります。
